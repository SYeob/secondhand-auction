...
const MyPage = () => {
  ...
  const fetchMyData = async (userId: string) => {
    ...
    try {
      // 내가 응찰한 상품들 가져오기 (seller_phone은 노출하지 않기 위해 컬럼 명시)
      const { data: bidsData, error: bidsError } = await supabase
        .from("bids")
        .select("product_id, products(id, title, location, current_price, image_url, end_time, category)")
        .eq("user_id", userId)
      ...
      if (!likesError && likesData && likesData.length > 0) {
        ...
        // 상품 정보 가져오기 (seller_phone은 노출하지 않음)
