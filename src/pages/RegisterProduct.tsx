...
const RegisterProduct = () => {
  ...
  useEffect(() => {
    ...
    // 수정 모드일 경우 기존 데이터 불러오기 (SECURITY DEFINER 함수를 통해 소유자만 seller_phone 접근 가능)
    const loadProductData = async () => {
      if (editId) {
        setIsEditMode(true);
        const { data, error } = await supabase.rpc('get_own_product', {
          p_product_id: editId,
        });
        ...
        setFormData({
          ...
          endTime: new Date(product.end_time).toISOString().slice(0, 16),
